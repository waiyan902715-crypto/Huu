...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "Example:\n`http://login.kyawwifi.com/login` သို့မဟုတ် `https://portal-as.ruijienetworks.com/...`"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # URL ပုံစံ မှန်မမှန် စစ်ဆေးခြင်း (http သို့မဟုတ် https စသည့် URL ဖြစ်ရန်)
    if text.startswith("http://") or text.startswith("https://"):
        if "mac=" not in text:
            separator = '&' if '?' in text else '?'
            text += f"{separator}mac=02:00:00:00:00:00"
        
        user_scans[user_id]['url'] = text
        await show_main_menu(update, context)
    else:
        await update.message.reply_text("❌ မှန်ကန်သော Portal URL မဟုတ်ပါ။ (http:// သို့မဟုတ် https:// ဖြင့် စတင်ပါ)")
